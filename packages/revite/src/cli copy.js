import getCommand, { CommandType } from "./commands"
...
async function resolveOptions({
  ...
  if (argv.sw || argv.serviceWorker) {
    console.warn(
      chalk.yellow(
        `[revite] service worker mode has been removed due to insufficient performance gains.`
      )
    )
  }
...
function runServe(options: UserConfig) {
  ...
  server.on('error', (e: Error & { code?: string }) => {
    if (e.code === 'EADDRINUSE') {
      ...
      console.error(chalk.red(`[revite] server error:`))
      console.error(e)
    }
  })
...
async function runBuild(options: UserConfig) {
  try {
    ...
    console.error(chalk.red(`[revite] Build errored out.`))
    console.error(err)
    process.exit(1)
  }
}
...
async function runOptimize(options: UserConfig) {
  try {
    ...
    console.error(chalk.red(`[revite] Dep optimization errored out.`))
    console.error(err)
    process.exit(1)
  }
}
